...
%total columns in A are n+lt+eq+2*gt

B_columns = [n+lt+eq+2*gt-m+1 : lt+2*gt+n+eq]; %columns of basic matrix
B = [A(1:m, B_columns)]; %basic matrix
...
Simplex % a matrix of size (m+1)*(n+lt+2*gt+eq+1);
